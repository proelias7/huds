    ...
    window.addEventListener('message', function(e) {
        ...
        if (data.action == 'player') {   
            if (data.needs) {
                ...
                if (data.needs.fome <= 15){
                    $('body > status3 > needs3 > fome > img').addClass('imgactive');
                }else {
                    $('body > status3 > needs3 > fome > img').removeClass('imgactive');
                }
                if (data.needs.sede <= 15){
                    $('body > status4 > needs4 > sede > img').addClass('imgactive');
                }else {
                    $('body > status4 > needs4 > sede > img').removeClass('imgactive');
                }

                if (data.needs.armor <= 0){
                    $('body > status2').addClass('hide');
                    $('body > status').css('bottom','119px');
                }else{
                    $('body > status2').removeClass('hide');
                    $('body > status').css('bottom','153px');
                }
                ...
                $('body > status3 > needs3 > fome > div').css('width', data.needs.fome + '%');
                $('body > status3 > needs3 > fome > span#fome > p').html(data.needs.fome + '%');
                
                $('body > status4 > needs4 > sede > div').css('width', data.needs.sede + '%');
                $('body > status4 > needs4 > sede > span#sede > p').html(data.needs.sede + '%');
            }
        }
